 ...
 */
export const FACE_DIRECTIONAL_FLOW: Record<FaceShapeType, DirectionalFlow> = {
  oval: "balance",
  round: "sky",
  square: "sky",
  rectangular: "sky",
...
const FACE_RULES: Record<FaceShapeType, Omit<HairDesign, "rationale">> = {
  round: {
 ...
 */
function applyNeckModifier(
  design: Omit<HairDesign, "rationale">,
  neckLength?: string,
): Omit<HairDesign, "rationale"> {
  if (neckLength === "short") {
    return {
      ...design,
      forbiddenLengths: [
 ...
 */
export function evaluateFaceMorphology(
  input: FaceMorphologyInput,
): HairDesign | null {
  const shape = input.faceShape?.toLowerCase() as FaceShapeType | undefined;
  if (!shape || !(shape in FACE_RULES)) return null;
...
function buildRationale(
  ...
  if (input.neckLength) parts.push(`neck: ${input.neckLength}`);
  if (input.shoulders) parts.push(`shoulders: ${input.shoulders}`);
  return parts.join(", ");
}
