...
export interface BodyStyleModifiers {
  lengthBias: string[]; // lengths that harmonise with this body type
  forbiddenLengths: string[]; // lengths that create imbalance
  volumeBias: string[]; // placement of volume that flatters
  forbiddenVolume: string[];
  rationale: string;
}

const BODY_RULES: Record<BodyType, BodyStyleModifiers> = {
  hourglass: {
    ...
    forbiddenVolume: [],
    rationale:
      "Hourglass: balanced proportions suit most lengths; emphasise symmetry",
  },
  ...
  triangle: {
    ...
    rationale:
      "Triangle: draw eye upward; avoid length and volume that fall at hip level",
  },
  ...
  inverted_triangle: {
    ...
    rationale:
      "Inverted triangle: add length to balance shoulders; avoid crown volume",
  },
};
 ...
 */
export function evaluateBodyMorphology(
  input: BodyMorphologyInput,
): BodyStyleModifiers | null {
  const bodyType = input.bodyType?.toLowerCase() as BodyType | undefined;
  if (!bodyType || !(bodyType in BODY_RULES)) return null;
