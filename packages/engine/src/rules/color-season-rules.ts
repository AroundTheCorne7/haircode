...
const SEASON_BASE: Record<ColorSeason, SeasonBase> = {
  spring: {
    recommendedTones: [
      "warm golden blonde",
      "strawberry blonde",
 ...
 */
function applyContrastModifiers(
  base: SeasonBase,
  contrastScore: ContrastScore,
): Pick<ColorStrategy, "recommendedTechniques" | "avoidTechniques"> {
 ...
 */
export function evaluateColorIdentity(
  input: ColorIdentityInput,
): ColorStrategy | null {
  const season = input.colorSeason?.toLowerCase() as ColorSeason | undefined;
  if (!season || !(season in SEASON_BASE)) return null;

  const base = SEASON_BASE[season];
  const { recommendedTechniques, avoidTechniques } = applyContrastModifiers(
    base,
    input.contrastScore,
  );
