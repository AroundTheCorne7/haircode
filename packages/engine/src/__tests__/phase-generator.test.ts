...
describe("assignPhase", () => {
  ...
  it("forces stabilization when CRITICAL red flag is present", () => {
    const criticalFlag = { severity: "CRITICAL" as const, code: "RF_TEST", message: "test", penaltyFactor: 0, requiresAcknowledgment: false };
    expect(assignPhase(80, [criticalFlag])).toBe("stabilization");
  });

  it("forces stabilization when BLOCK red flag is present", () => {
    const blockFlag = { severity: "BLOCK" as const, code: "RF_TEST", message: "test", penaltyFactor: 0, requiresAcknowledgment: false };
