 ...
 */
export function resolveArchetypeStyle(blend) {
    ...
    const preferredLengths = mergeWeighted(primary.preferredLengths, secondary.preferredLengths, pW, sW, primary.avoidLengths);
    const preferredShapes = mergeWeighted(primary.preferredShapes, secondary.preferredShapes, pW, sW, primary.avoidShapes);
    // Avoid list is intersection — only avoid what both archetypes avoid
    const avoidLengths = primary.avoidLengths.filter((l) => secondary.avoidLengths.includes(l));
    const avoidShapes = primary.avoidShapes.filter((s) => secondary.avoidShapes.includes(s));
    // Maintenance: take the higher maintenance level when blending
    const maintenanceLevel = higherMaintenance(primary.maintenanceLevel, secondary.maintenanceLevel);
    return {
        preferredLengths,
        preferredShapes,
        avoidLengths,
...
function mergeWeighted(primaryList, secondaryList, pW, sW, avoidList) {
    const combined = pW >= sW
        ? [
            ...primaryList,
...
function higherMaintenance(a, b) {
    return MAINTENANCE_ORDER[a] >= MAINTENANCE_ORDER[b] ? a : b;
}
//# sourceMappingURL=archetype-rules.js.map