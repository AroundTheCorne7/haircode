...
describe("evaluateRedFlags", () => {
    it("blocks on open lesions", () => {
        const { flags, blocked } = evaluateRedFlags({ clientId: "test", scalp: { openLesions: true } }, RED_FLAG_RULES);
        expect(blocked).toBe(true);
        expect(flags.find((f) => f.code === "RF_SCALP_007")).toBeDefined();
    });
    it("flags damage index 10 as critical", () => {
        const { flags } = evaluateRedFlags({ clientId: "test", hair: { damageIndex: 10 }, scalp: {} }, RED_FLAG_RULES);
        expect(flags.find((f) => f.code === "RF_HAIR_001")).toBeDefined();
    });
    it("returns no flags for a healthy profile", () => {
        const { flags, blocked } = evaluateRedFlags({ clientId: "test", scalp: { openLesions: false }, hair: { damageIndex: 2 }, body: {} }, RED_FLAG_RULES);
        expect(blocked).toBe(false);
        expect(flags).toHaveLength(0);
    });
    it("blocks on chemotherapy medication", () => {
        const { blocked } = evaluateRedFlags({ clientId: "test", body: { medication: ["chemotherapy"] }, scalp: {}, hair: {} }, RED_FLAG_RULES);
        expect(blocked).toBe(true);
    });
});
//# sourceMappingURL=red-flag.test.js.map