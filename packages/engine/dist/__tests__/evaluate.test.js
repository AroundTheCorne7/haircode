...
import { evaluate, DEFAULT_RULES, DEFAULT_WEIGHTS, } from "../index.js";
const NORMALIZERS = [
...
const RED_FLAG_RULES = [
    ...
    // §5.6 — RF_SCALP_006 — Seborrheic + Elevated pH (CRITICAL)
    {
        ...
        condition: (profile) => Array.isArray(profile.scalp?.conditions) &&
            profile.scalp.conditions.includes("seborrheic") &&
            typeof profile.scalp?.phLevel === "number" &&
            profile.scalp.phLevel > 6.0,
    },
    // §5.6 — RF_HAIR_001 — Severe Damage (CRITICAL)
    {
        ...
        condition: (profile) => typeof profile.hair?.damageIndex === "number" &&
            profile.hair.damageIndex >= 10,
    },
];
// ---------------------------------------------------------------------------
...
describe("Spec §5.9 — Scenario A: damageIndex=10, openLesions=true → BLOCKED", () => {
    ...
    it("result has a string evaluationId (UUID)", async () => {
        ...
        // UUID v4 regex
        expect(result.evaluationId).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i);
    });
});
// ---------------------------------------------------------------------------
...
describe("Spec §5.7 — DEFAULT_RULES case sensitivity", () => {
    ...
    it('rule-004: porosity="high" (lowercase) does NOT trigger (condition is "HIGH")', async () => {
        ...
        const hasRule004Action = result.appliedActions.some((a) => a.type === "ADJUST_SCORE" &&
            a.target === "hair" &&
            a.value === -5);
    ...
    it('rule-003: biotype="seborrheic" (lowercase) does NOT trigger (condition is "SEBORRHEIC")', async () => {
        ...
        const hasRule003Action = result.appliedActions.some((a) => a.type === "TRIGGER_ALERT" &&
...
describe("Spec §5.10 — EvaluationResult shape", () => {
    it("result includes all required top-level fields", async () => {
        ...
        expect(["stabilization", "transformation", "integration"]).toContain(result.assignedPhase);
