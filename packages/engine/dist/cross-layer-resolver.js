 ...
 */
export function resolveLayers(decisions) {
    const conflicts = [];
    // ── Hair Design: start from face morphology (L1), refine with body (L3) ────
    const hairDesign = buildHairDesign(decisions.faceMorphology, decisions.bodyMorphology, decisions.archetypeStyle, conflicts);
    ...
    const colorWithFlags = applyRedFlagToColor(colorStrategy, decisions.redFlags, conflicts);
    // ── Technical Roadmap: derived from phase + score ─────────────────────────
    const technicalRoadmap = buildTechnicalRoadmap(decisions.assignedPhase, decisions.analysisScore, decisions.redFlags);
    // ── Treatment Protocol: from red flags + hair structure ───────────────────
    const treatmentProtocol = buildTreatmentProtocol(decisions.redFlags, decisions.analysisScore);
    return {
        hairDesign,
...
// ─── Hair Design Merger ────────────────────────────────────────────────────────
function buildHairDesign(face, body, archetype, conflicts) {
    ...
    if (archetype) {
        ...
        const safeArchetypeLengths = archetype.preferredLengths.filter((l) => !forbiddenLengths.includes(l));
...
// ─── Color Strategy ────────────────────────────────────────────────────────────
function applyRedFlagToColor(strategy, redFlags, conflicts) {
    const additionalContraindications = [];
    for (const flag of redFlags) {
        if (flag.severity === "BLOCK") {
            additionalContraindications.push(`BLOCK (${flag.code}): All chemical services contraindicated — ${flag.message}`);
            conflicts.push({
            ...
            additionalContraindications.push(`CRITICAL (${flag.code}): Chemical services require caution — ${flag.message}`);
        }
    }
    return {
        ...strategy,
...
function buildDefaultColorStrategy() {
    return {
        season: "summer",
        contrastScore: 3,
...
function buildTechnicalRoadmap(phase, score, redFlags) {
...
function getPhaseServices(phase, phaseIndex) {
    const services = {
        stabilization: [
...
function getPhaseCheckpoints(phase, phaseIndex, score) {
...
function buildTreatmentProtocol(redFlags, score) {
    const homecare = [
    ...
    if (score <= 40) {
        homecare.push("Daily scalp oil (non-comedogenic)", "Protein treatment every 2 weeks");
        inSalon.push("Intensive repair treatment (every visit)");
    }
    // Add red-flag contraindications
    for (const flag of redFlags) {
        if (flag.severity === "BLOCK") {
            contraindications.push(`No chemical services: ${flag.message} (${flag.code})`);
        }
        else if (flag.severity === "CRITICAL") {
            contraindications.push(`Caution required: ${flag.message} (${flag.code})`);
        }
    }
    return { homecare, inSalon, contraindications };
}
//# sourceMappingURL=cross-layer-resolver.js.map