...
export function computeModuleScores(profile, normalizers, weights) {
    const moduleKeys = ["hair", "scalp", "body", "morphology"];
    const scores = { hair: 50, scalp: 50, body: 50, morphology: 50 };
    for (const module of moduleKeys) {
        ...
        for (const normalizer of moduleNormalizers) {
            ...
            const rawValue = getNestedValue(moduleData, fieldName);
...
export function computeCompositeScore(moduleScores, moduleWeights) {
    let total = 0;
    let weightSum = 0;
