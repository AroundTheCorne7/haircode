...
async function getTokenForOtherTenant(): Promise<string> {
  const { getTestToken } = await import("../test-helpers/auth.js");
  return getTestToken({
    role: "practitioner",
...
describe("GET /clients", () => {
  ...
  it("returns only clients belonging to the authenticated tenant", async () => {
    ...
    const createdClient = createRes.json<{ data: { id: string; tenantId: string } }>().data;
    ...
    const { data: tenantBClients } = listRes.json<{ data: Array<{ id: string; tenantId: string }> }>();
  ...
  it("search parameter longer than 200 characters → 400 INVALID_PARAM", async () => {
    ...
    expect(body.error.message).toBe(
      "search must be a string of max 200 characters",
    );
  });
...
describe("POST /clients", () => {
  ...
  it("gdprConsentGiven: false → 422 VALIDATION_ERROR", async () => {
    ...
    expect(
      (fieldErrors?.gdprConsentGiven as string[]).some((msg: string) =>
        msg.includes("GDPR consent must be explicitly granted"),
      ),
    ).toBe(true);
  });
...
describe("GET /clients/:id", () => {
  ...
  it("valid UUID id that belongs to a different tenant → 404 NOT_FOUND", async () => {
    ...
    expect(createRes.statusCode).toBe(201);
    const clientId = createRes.json<{ data: { id: string } }>().data.id;
