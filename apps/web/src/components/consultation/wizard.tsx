  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="border-b border-gray-100 px-4 sm:px-6 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
        <button onClick={() => window.history.back()} className="p-2 -m-2 text-sm text-gray-500 hover:text-gray-900 min-h-[44px] flex items-center">
          ✕ Cancel
        </button>
      ...
      <div className="flex-1 px-4 sm:px-6 py-4 sm:py-6 max-w-2xl mx-auto w-full">
        {currentStep === 1 && <Step1Profile data={data} onUpdate={updateData} onNext={next} />}
        {currentStep === 2 && <Step2Hair data={data} onUpdate={updateData} onNext={next} onBack={back} />}
        {currentStep === 3 && <Step3Scalp data={data} onUpdate={updateData} onNext={next} onBack={back} />}
        {currentStep === 4 && <Step4Body data={data} onUpdate={updateData} onNext={next} onBack={back} />}
        {currentStep === 5 && <Step5Morphology data={data} onUpdate={updateData} onNext={next} onBack={back} />}
        {currentStep === 6 && <Step6Protocol data={data} onBack={back} />}
      </div>
    </div>
  );
}
