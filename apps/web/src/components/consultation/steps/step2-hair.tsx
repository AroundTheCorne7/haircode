    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Structure, condition, and history
        </p>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {HAIR_TYPES.map((t) => (
            <label
              key={t}
              className="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg cursor-pointer hover:border-brand transition-colors has-[:checked]:border-brand has-[:checked]:bg-brand/5"
            >
              <input
                {...register("hairType")}
                type="radio"
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {TEXTURES.map((t) => (
            ...
            >
              <input
                {...register("texture", {
                  required: "Please select a hair texture",
                })}
                type="radio"
                value={t.toLowerCase()}
                className="text-brand"
        ...
        {(errors as Record<string, { message?: string }>).texture && (
          <p className="mt-1 text-xs text-red-500">
            {(errors as Record<string, { message?: string }>).texture?.message}
          </p>
        )}
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {[
            "Colour-treated (permanent)",
            "Colour-treated (semi/demi)",
            "Bleach / Lightening",
            "Relaxer / Keratin",
            "Thermal damage (regular heat)",
          ].map((item) => (
            <label
              key={item}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Overall Damage Index (0–10)
        </label>
        <input
          {...register("damageIndex", { valueAsNumber: true })}
          type="range"
          min={0}
          max={10}
          step={1}
